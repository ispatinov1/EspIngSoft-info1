...
if($_GET['id'] && !$_POST['btnModificarTipoDocumento']){
    $resVerTipoDocumento = $controlador->ver($_GET['id']);    
}
if($_POST['btnModificarTipoDocumento']){     
    $resModificarTipoDocumento = $controlador->editar($_POST['nbrIdTipoDocumento'], $_POST['txtNombreTipoDocumento'], $_POST['txtDescripcionTipoDocumento']);    
    $resVerTipoDocumento = $controlador->ver($_GET['id']); 
}

$titulo = utf8_encode('Gestión de tipos de documento');         
$subTitulo = utf8_encode('Modificar tipos de documento');
$inicio = 'index.php';
$listar = '?carga=listar';
$crear  = '?carga=crear';
$buscar = '?carga=buscar';    
...
$smarty->assign('resultado'     , $resVerTipoDocumento);
