...
if($_GET['id'] && !$_POST['btnModificarMotivoTipoSolicitud']){
    $resVerMotivosTipoSolicitud = $controlador->ver($_GET['id']);    
}
if($_POST['btnModificarMotivoTipoSolicitud']){     
    $resModificarMotivosTipoSolicitud = $controlador->editar($_POST['nbrIdMotivoTipoSolicitud'], $_POST['txtNombreMotivoTipoSolicitud'], $_POST['txtDescripcionMotivoTipoSolicitud']);    
    $resVerMotivosTipoSolicitud = $controlador->ver($_GET['id']); 
}

$titulo = utf8_encode('Gestión de motivos por tipo de solicitud');         
$subTitulo = utf8_encode('Modificar motivos para tipo de solicitud');
$inicio = 'index.php';
$listar = '?carga=listar';
$crear  = '?carga=crear';
$buscar = '?carga=buscar';    
...
$smarty->assign('resultado'     , $resVerMotivosTipoSolicitud);
