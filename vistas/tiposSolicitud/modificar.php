...
if($_GET['id'] && !$_POST['btnModificarMotivoTipoSolicitud']){
    $resVerTiposDocumento = $controlador->ver($_GET['id']);    
}
if($_POST['btnModificarMotivoTipoSolicitud']){     
    $resModificarTiposDocumento = $controlador->editar($_POST['nbrIdMotivoTipoSolicitud'], $_POST['txtNombreMotivoTipoSolicitud'], $_POST['txtDescripcionMotivoTipoSolicitud']);    
    $resVerTiposDocumento = $controlador->ver($_GET['id']); 
}

$titulo = utf8_encode('Gestión de tipos de documento');         
$subTitulo = utf8_encode('Modificar tipo de documento');
$inicio = 'index.php';
$listar = '?carga=listar';
$crear  = '?carga=crear';
$buscar = '?carga=buscar';    
...
$smarty->assign('resultado'     , $resVerTiposDocumento);
...
$smarty->display($seccion . 'modificar.tpl'); 
