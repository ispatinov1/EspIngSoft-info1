...
if($_GET['id'] && !$_POST['btnEliminarTipoDocumento']){
    $resVerTiposDocumento = $controlador->ver($_GET['id']);    
}
else{     
    $resEliminarTipoDocumento = $controlador->eliminar($_POST['nbrIdTipoDocumento']);    
    //$resVerTipoDocumento = $controlador->ver($_GET['id']); 
}

$titulo = utf8_encode('Gestión de tipos de documento');        
$subTitulo = utf8_encode('¿Está seguro de querer eliminar este registro?');
$inicio = 'index.php';
$listar = '?carga=listar';
$crear  = '?carga=crear';
$buscar = '?carga=buscar';    
...
$smarty->display($seccion . 'eliminar.tpl'); 
